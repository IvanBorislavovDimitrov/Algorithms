...
public class DistancesBetweenVertices {
    ...
    public static void main(String[] args) throws IOException {
        ...
        List<AbstractMap.SimpleEntry<Integer, Integer>> pairs = pairsToFind(pairsCount);
    ...
    private static List<AbstractMap.SimpleEntry<Integer, Integer>> pairsToFind(int pairsCount) throws IOException {
        List<AbstractMap.SimpleEntry<Integer, Integer>> pairs = new ArrayList<>();
        for (int i = 0; i < pairsCount; i++) {
            String[] pair = bufferedReader.readLine().split("-");
            int startNode = Integer.parseInt(pair[0]);
            int endNode = Integer.parseInt(pair[1]);
            pairs.add(new AbstractMap.SimpleEntry<>(startNode, endNode));
        }
        return pairs;
    }

    private static void printShortestDistances(Map<Integer, List<Integer>> graph, List<AbstractMap.SimpleEntry<Integer, Integer>> pairs) {
        pairs.forEach((pair) -> {
