...
public class AreasInMatrix {
    ...
    public static void main(String[] args) throws IOException {
        ...
        AbstractMap.SimpleEntry<Integer, Integer> unvisitedElement = getFirstUnvisitedElements(visited);
    ...
    private static AbstractMap.SimpleEntry<Integer, Integer> getFirstUnvisitedElements(boolean[][] visited) {
        for (int i = 0; i < visited.length; i++) {
            for (int i1 = 0; i1 < visited[i].length; i1++) {
                if (!visited[i][i1]) {
                    return new AbstractMap.SimpleEntry<>(i, i1);
                }
            }
        }
        return null;
